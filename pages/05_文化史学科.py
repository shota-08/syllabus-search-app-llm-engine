...
# チャットログセッションの初期化
if "bunkashi_chat_log" not in st.session_state:
    st.session_state.bunkashi_chat_log = []
...
if query:
    ...
    # ログの表示
    for chat in st.session_state.bunkashi_chat_log:
    ...
    # セッションにログを追加
    st.session_state.bunkashi_chat_log.append({"name" : USER_NAME, "msg" : query})
    st.session_state.bunkashi_chat_log.append({"name" : ASSISTANT_NAME, "msg" : answer})